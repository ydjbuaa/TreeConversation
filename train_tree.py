...
def memory_efficient_loss(outputs, targets, generator, crit, eval_flag=False):
    ...
    for i, (out_t, tgt_t) in enumerate(zip(outputs_split, targets_split)):
        ...
        loss_t = crit(scores_t.contiguous(), tgt_t.contiguous().view(-1))
...
def train_model(model, train_set, valid_set, vocabs, optim):
    ...
    def train_epoch(epoch):
        ...
        for i in range(len(train_set)):
            ...
            #print(src_batch[0].size(), src_batch[1].size(), tgt_batch.size())
...
def main():
    ...
    train_set = TreeDataset(dataset['train']['src'],
                            dataset['train']['trans'],
                            dataset['train']['tgt'],
                            configs['batch_size'],
                            cuda=True,
    ...
    encoder = EncoderTreeLSTM(configs, vocabs['src'].size)
