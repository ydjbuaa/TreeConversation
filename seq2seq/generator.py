...
class ConversationGenerator(object):
    def __init__(self, opt):
        ...
        encoder = EncoderRNN(model_config, self.src_vocab.size)
        decoder = AttnDecoderRNN(model_config, self.tgt_vocab.size)
        generator = nn.Linear(model_config['rnn_hidden_size'], self.tgt_vocab.size)


        #model = Seq2SeqChatbot(encoder, decoder, generator)
        model = Seq2SeqModel(encoder, decoder)

        # generator = nn.Sequential(
        #     nn.Linear(model_config['rnn_hidden_size'], self.tgt_vocab.size),
        #     nn.LogSoftmax())
    ...
    def generate_conversation_batch(self, src_batch, tgt_batch):
        ...
        if isinstance(enc_states, tuple) or isinstance(enc_states, list):
            enc_states = (self.model._fix_enc_hidden(enc_states[0]),
                          self.model._fix_enc_hidden(enc_states[1]))
        else:
            enc_states = self.model._fix_enc_hidden(enc_states)
